...
| 2026-09-07 | v0.2.6 | **M9 增强：跳转重构（VS Code 设计）+ 预览标签页**：跳转逻辑从 commands.ts 拆为 navigation.ts（流程）/navigationStore.ts（历史栈+Peek 状态）/PeekView.tsx（编辑器内 Peek 浮层）；多定义/引用 Peek 呈现（锚定光标行、按文件分组、上下文预览），jump 面板模式移除；新增后退/前进（Alt+←/→）、查看定义 Peek（Alt+F12）、光标词高亮（wordHighlighter）；预览标签（EditorTab.preview：单击斜体、未修改原位替换、变脏/双击/保持打开转常驻）。质量门：build/cargo check 绿；UI 走查 T1 全过、T2 部分（余项见 TODO_20260905 移交） |
| 2026-09-07 | v0.2.7 | **FR-06 终端多 Shell**：Rust terminal.rs 新增 `list_terminal_shells` 探测命令（Git Bash：注册表/PATH git.exe 上溯/常见路径三级；pwsh、WSL 经 PATH，WSL 需已装发行版）+ `create_terminal` shell 参数（不可用回退 PowerShell；reg.exe 方案零新增依赖）；前端 Panel 标签栏 Shell 下拉选择器（选中即建会话并记忆默认）、terminalStore 增 shells/loadShells/resolveDefaultShell；默认 Shell 落 settings.json（terminalShell 字段，serde default 兼容旧配置）；命令面板「新建终端」「运行活动文件」跟随默认 Shell。质量门：build/clippy -D warnings/fmt 绿 |
| 2026-09-07 | v0.2.8 | **FR-09 动态语言高亮**：monaco-setup 增 `registerMonarchLanguage`（运行时注册 Monaco 语言 + 扩展名映射，同 id 用户定义覆盖内置）；Rust `get/set_user_languages`（languages.json 原样存取）；languageStore 启动加载/增删/持久化；LanguageManager 弹窗（列表+添加表单）经命令面板「管理语言高亮」打开，palette kind 扩展 languages（CommandPalette 排除该 kind 自渲染）。TextMate→Monarch 转换器范围外。质量门：build/clippy/fmt 绿 |
...
| 2026-09-19 | v0.3.0 | **M10 范围收缩收口**：移除 L3 扩展命令执行（`main.js` 沙箱 + `vscode` 垫片，registry.ts 795→253 行）与在线插件市场（FR-16）；新增安全性增强（VSIX 路径/归属校验、解包上限、文件作用域、命令命名空间守卫、主题色值白名单、CSP 启用）；文档同步 REQUIREMENTS §5/§6/NFR-03/NFR-06 与 README/AGENTS §7。质量门：npm run build / cargo clippy -D warnings / cargo fmt / cargo test（20 例）全绿 |
