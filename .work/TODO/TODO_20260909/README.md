...
- **验收标准**：
  ...
  - [ ] `npm run tauri dev` 人工走查：活动栏入口、过滤、展开文件、Diff、加载更多、非仓库空态（需用户侧确认）
...
- 三项改动仍需 `npm run tauri dev` 真指针走查：拖侧栏、拖面板、双击重置、Tab 到手柄后方向键微调、`Alt+←/→` 不被吞。
