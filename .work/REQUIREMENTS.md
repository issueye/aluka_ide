...
| FR-21 | 提交记录（Git 历史） | 独立侧栏视图（活动栏入口 + 命令面板「提交记录（Git 历史）」）：当前分支提交列表（提交信息/作者/邮箱/相对时间/短哈希，悬停显完整哈希）；顶部查询框按提交信息/作者/哈希实时子串过滤；点击提交展开该提交相对**首父**（根提交相对空树）改动的文件清单（M/A/D 徽标，重命名拆为 D+A），点击文件打开提交前后 Monaco Diff；加载更多按 100/档扩展、上限 1000 条；空仓库返回空列表、非仓库空态可一键 `git init`；实现为 Rust `git_log`/`git_commit_detail` 两条命令（git log `\x1f` 单行解析；`--name-status -z` NUL 成对解析防路径转义） | P2 | Git 仓库内列表可见且可按关键词过滤；展开可见改动文件并可打开提交前后对比；根提交（对比空树）与合并提交（仅对比第一父）不报错 |
| FR-22 | 终端 IO 调试（FR-06 配套） | 在 xterm → Rust `write_terminal` 之间注入可插拔调试管道（`terminalIo.ts`）：会话级模式（放行/丢弃整帧拦截/规则改写）、双向记录（输入+输出，环形缓冲上限 1000 条）、手动发送（转义解析 `\x1b`/`\r` 等）、历史重发、复制原文/UTF-8 十六进制；常驻侧栏「终端 IO 调试」视图（顶端「终端」菜单唯一入口 + 活动栏图标随开随显、标题栏 × 彻底关闭）；调试状态**不持久化**、默认不影响正常终端输入（关闭即复位拦截/改写/记录） | P2 | 打开调试视图自动录制；丢弃模式输入不再发送；改写规则生效；手动发送/历史重发可达终端；反复开关无状态残留；关闭后终端输入不受影响 |
